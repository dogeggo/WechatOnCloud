...
export function InstanceVncServerProfile({
  ...
  const { toast } = useUI();
  const [profile, setProfile] = useState<VncServerProfile>(
    inst.vncServerProfile,
  );
  ...
  const submit = async (event: FormEvent) => {
    ...
    setBusy(true);
    try {
      const { instance } = await api.setInstanceVncServerProfile(
        inst.id,
        profile,
      );
  ...
  return (
    <div className="modal-mask" onClick={onClose}>
      <form
        ...
        style={{ maxWidth: 500 }}
      >
        <h2>VNC编码 · {inst.name}</h2>
        <div className="muted small" style={{ lineHeight: 1.6 }}>
          这里调整 KasmVNC
          服务端编码参数。保存后会重建该实例容器，当前远程连接会断开，数据卷和应用数据保留。
        </div>
        ...
        <div className="vnc-profile-list">
          {VNC_SERVER_PROFILE_OPTIONS.map((option) => (
            <button
              ...
              key={option.profile}
              className={
                "vnc-profile-option" + (profile === option.profile ? " on" : "")
              }
            ...
            >
              <span className="vnc-profile-top">
                <b>{option.label}</b>
                {inst.vncServerProfile === option.profile && (
                  <span className="tag tag-on">当前</span>
                )}
              </span>
        ...
        <div className="modal-actions">
          ...
          <button
            ...
            disabled={busy || profile === inst.vncServerProfile}
          >
            {busy ? "保存并重启中..." : "保存并重启实例"}
          </button>
        </div>
